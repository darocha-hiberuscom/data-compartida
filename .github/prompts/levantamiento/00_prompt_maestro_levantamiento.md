...
| 6 | [06_resumen_servicios_dtos.md](./06_resumen_servicios_dtos.md) | Generar el resumen global de servicios, DTOs y JSON request del orquestador. | Secciones 12.0–14.0 |
...
- **Fuente de dependencias**: en `repos/origen/servicios-dependencia` se encuentran los archivos con la información de la URL del servicio y el request que se arma para invocarlo.
- A partir de ambas fuentes, documentar por cada servicio integrado: nombre del servicio (extraído del ESQL), URL/endpoint (extraído de servicios-dependencia) y estructura del request (extraída de servicios-dependencia).
...
2. La URL o endpoint del servicio (buscar en `repos/origen/servicios-dependencia`).
...
- URL del servicio: buscar en los archivos de `repos/origen/servicios-dependencia` la configuración del endpoint asociado.
...
- Los XML SOAP de origen y destino se encuentran ambos en `repos/origen/servicios-dependencia`, identificados por el nombre del archivo:
...
- Si no se encuentra el XML de ejemplo en servicios-dependencia, marcar `FALTA DETALLE` con la pista del procedimiento.

#### 4.8 Mapeo de entrada del orquestador (OBLIGATORIO)
Documentar todos los campos que recibe el orquestador como entrada (request del orquestador), identificando la ruta completa del campo, el tipo de dato y su path en el XML SOAP del orquestador (obtenido de `repos/origen/servicios-dependencia`, archivo con el nombre del orquestador).
...
| # | Código error | Mensaje de error | Tipo | Origen (procedimiento/función) | Condición que lo genera | Línea/evidencia |
|---|--------------|------------------|------|-------------------------------|-------------------------|-----------------|
